...
router.get('/promos', (req, res) => {
  ...
  const promos = db.prepare(`
    SELECT p.*,
      ...
      COALESCE(SUM(u.order_amount), 0) AS total_sales,
      COALESCE(SUM(u.reward_amount), 0) AS total_reward
    FROM promo_codes p
...
router.post('/promos', (req, res) => {
  const db = getDb();
  const { code, partner_name, discount_pct, telegram_chat_id } = req.body;
  if (!code || !partner_name) return res.status(400).json({ error: 'Укажите код и имя партнёра' });
  try {
    const r = db.prepare(
      'INSERT INTO promo_codes (code, partner_name, discount_pct, telegram_chat_id) VALUES (upper(?), ?, ?, ?)'
    ).run(code.trim(), partner_name.trim(), parseFloat(discount_pct) || 3, telegram_chat_id || null);
    logAction(db, 'promo_create', `Создан промокод ${code.toUpperCase()} (${partner_name})`, req);
    res.json({ id: r.lastInsertRowid });
  } catch (e) {
    if (e.message.includes('UNIQUE')) return res.status(400).json({ error: 'Такой промокод уже существует' });
    throw e;
  }
});
...
router.put('/promos/:id', (req, res) => {
  const db = getDb();
  const { code, partner_name, discount_pct, active, telegram_chat_id } = req.body;
  db.prepare(`UPDATE promo_codes SET code=upper(?), partner_name=?, discount_pct=?, active=?, telegram_chat_id=? WHERE id=?`
  ).run(code, partner_name, parseFloat(discount_pct) || 3, active ? 1 : 0, telegram_chat_id || null, req.params.id);
  logAction(db, 'promo_update', `Обновлён промокод ID ${req.params.id}`, req);
  res.json({ ok: true });
});
...
router.delete('/promos/:id', (req, res) => {
  ...
  logAction(db, 'promo_delete', `Удалён промокод ID ${req.params.id}`, req);
  res.json({ ok: true });
});

// Использования конкретного промокода
router.get('/promos/:id/usages', (req, res) => {
  const db = getDb();
  const usages = db.prepare(`
    SELECT u.*, l.name lead_name, l.phone lead_phone
    FROM promo_usages u
